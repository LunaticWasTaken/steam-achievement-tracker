...
private:
  ...
  public:
    ...
    const int32_t root_offset_x = ThemeManager::get_singleton()->get<int32_t>("root/offset_x", 0);
    const int32_t root_offset_y = ThemeManager::get_singleton()->get<int32_t>("root/offset_y", 0);
    const uint32_t root_width = ThemeManager::get_singleton()->get<int32_t>("root/width", 282);
    const uint32_t root_height = ThemeManager::get_singleton()->get<int32_t>("root/height", 70);
    const std::string root_background = expand_vars(ThemeManager::get_singleton()->get<std::string>("root/background", "rgba(25,25,25,1.0)"));
    const uint8_t root_rounding = ThemeManager::get_singleton()->get<uint8_t>("root/rounding", 0);
    ...
    const int32_t icon_offset_x = ThemeManager::get_singleton()->get<int32_t>("icon/offset_x", 11);
    const int32_t icon_offset_y = ThemeManager::get_singleton()->get<int32_t>("icon/offset_y", 11);
    const uint32_t icon_width = ThemeManager::get_singleton()->get<int32_t>("icon/width", 48);
    const uint32_t icon_height = ThemeManager::get_singleton()->get<int32_t>("icon/height", 48);
    const uint8_t icon_rounding = ThemeManager::get_singleton()->get<uint8_t>("icon/rounding", 0);
    ...
    const bool title_font_wrapping = ThemeManager::get_singleton()->get<bool>("title/font_wrapping", false);
